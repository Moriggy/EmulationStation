	...
	mSystem(system), mMenu(window, "OPCIONES"), fromPlaceholder(false), mFiltersChanged(false)
{
	...
	if (!fromPlaceholder) {
		...
		mJumpToLetterList = std::make_shared<LetterList>(mWindow, "SALTAR A...", false);
		...
		row.addElement(std::make_shared<TextComponent>(mWindow, "SALTAR A...", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
		...
		// sort list by
		mListSort = std::make_shared<SortList>(mWindow, "CLASIFICAR JUEGOS POR", false);
		...
		mMenu.addWithLabel("CLASIFICAR JUEGOS POR", mListSort);
	}
	// show filtered menu
	if(!Settings::getInstance()->getBool("ForceDisableFilters"))
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "FILTRAR GAMELIST", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "AGREGAR/QUITAR JUEGOS A ESTA COLECCION DE JUEGO", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "TERMINAR EDICION DE '" + Utils::String::toUpper(CollectionSystemManager::get()->getEditingCollection()) + "' COLLECCION", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);

	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "SCRAPEO EN PROCESO", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	mSubtitle = std::make_shared<TextComponent>(mWindow, "texto subtitulo", Font::get(FONT_SIZE_SMALL), 0x888888FF, ALIGN_CENTER);
	...
	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "PARAR", "PARAR (proceso guardado)", std::bind(&GuiScraperMulti::finish, this)));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "NO SE SCRAPEARON JUEGOS.";
	}else{
		ss << mTotalSuccessful << " JUEGO" << ((mTotalSuccessful > 1) ? "S" : "") << " SCRAPEADOS CON EXITO!";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << " JUEGO" << ((mTotalSkipped > 1) ? "S" : "") << " OMITIDO.";
	}

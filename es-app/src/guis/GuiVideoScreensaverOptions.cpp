...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("INTERCAMBIAR VIDEO DESPUÉS DE (SEGS)", swap);
	addSaveFunc([swap] {
	...
	addWithLabel("ESTIRAR VIDEO EN SCREENSAVER", stretch_screensaver);
	...
	addWithLabel("USAR OMX PLAYER PARA SCREENSAVER", ss_omx);
	...
	auto ss_info = std::make_shared< OptionListComponent<std::string> >(mWindow, "VER INFORMACION DEL JUEGO EN SCREENSAVER", false);
	std::vector<std::string> info_type;
	info_type.push_back("siempre");
	info_type.push_back("principio y final");
	info_type.push_back("nunca");
	for(auto it = info_type.cbegin(); it != info_type.cend(); it++)
		ss_info->add(*it, *it, Settings::getInstance()->getString("ScreenSaverGameInfo") == *it);
	addWithLabel("VER INFORMACION DEL JUEGO EN SCREENSAVER", ss_info);
	...
	auto ss_omx_subs_align = std::make_shared< OptionListComponent<std::string> >(mWindow, "ALINEACION DE LA INFORMACION DEL JUEGO", false);
	std::vector<std::string> align_mode;
	align_mode.push_back("izquierda");
	align_mode.push_back("centrada");
	...
	addWithLabel("ALINEACION DE LA INFORMACION DEL JUEGO", ss_omx_subs_align);
	...
	addWithLabel("TAMAÑO FUENTE INFORMACION DEL JUEGO", ss_omx_font_size);
	...
	addEditableTextComponent(row, "RUTA DE LA FUENTE", ss_omx_font_file, Settings::getInstance()->getString("SubtitleFont"));
	...
	addEditableTextComponent(row, "RUTA DE LA FUENTE CURSIVA", ss_omx_italic_font_file, Settings::getInstance()->getString("SubtitleItalicFont"));
	...
	addWithLabel("UTILIZAR UNA RESOLUCION BAJA COMPATIBLE PARA SUBTÍTULOS", captions_compatibility);
...
{
#ifdef _RPI_
	bool startingStatusNotRisky = (Settings::getInstance()->getString("ScreenSaverGameInfo") == "nunca" || !Settings::getInstance()->getBool("ScreenSaverOmxPlayer"));
#endif
	GuiScreensaverOptions::save();

#ifdef _RPI_
	bool endStatusRisky = (Settings::getInstance()->getString("ScreenSaverGameInfo") != "nunca" && Settings::getInstance()->getBool("ScreenSaverOmxPlayer"));

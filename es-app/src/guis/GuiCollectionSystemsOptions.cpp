...
GuiCollectionSystemsOptions::GuiCollectionSystemsOptions(Window* window) : GuiComponent(window), mMenu(window, "AJUSTES DE COLECCION DE JUEGO")
{
	initializeMenu();
}
...
void GuiCollectionSystemsOptions::initializeMenu()
{
	...
	{
		addEntry("CREAR NUEVA COLECCIÓN PERSONALIZADA DESDE EL THEME", 0x777777FF, true,
		[this, unusedFolders] {
			auto s = new GuiSettings(mWindow, "SELECCIONA LA CARPETA DEL THEME");
			std::shared_ptr< OptionListComponent<std::string> > folderThemes = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECCIONA LA CARPETA DEL THEME", true);
	...
	ComponentListRow row;
	row.addElement(std::make_shared<TextComponent>(mWindow, "CREAR NUEVA COLECCION PERSONALIZADA", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	row.makeAcceptInputHandler([this, createCustomCollection] {
		mWindow->pushGui(new GuiTextEditPopup(mWindow, "Nuevo nombre de la colecciOn", "", createCustomCollection, false));
	});
	...
	mMenu.addWithLabel("GRUPO ILIMITADO DE COLECCIONES PERSONALIZADAS", bundleCustomCollections);
	...
	mMenu.addWithLabel("CLASIFICAR COLECCIONES Y SISTEMAS PERSONALIZADOS", sortAllSystemsSwitch);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "EDICION FINAL '" + Utils::String::toUpper(CollectionSystemManager::get()->getEditingCollection()) + "' COLECCION", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
...
void GuiCollectionSystemsOptions::addSystemsToMenu()
{
	...
	autoOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECCIONA COLECCION", true);
	...
	mMenu.addWithLabel("COLECCION AUTOMATICA DE JUEGO", autoOptionList);

	std::map<std::string, CollectionSystemData> customSystems = CollectionSystemManager::get()->getCustomCollectionSystems();

	customOptionList = std::make_shared< OptionListComponent<std::string> >(mWindow, "SELECCIONA COLECCION", true);

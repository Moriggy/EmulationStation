	...
	mBackground(window, ":/frame.png"), mGrid(window, Vector2i(1, 5))
{
	...
	// title
	mTitle = std::make_shared<TextComponent>(mWindow, firstRun ? "BIENVENIDO" : "CONFIGURAR INPUT (MANDO)",
	...
	if(numDevices > 0)
		deviceInfo << numDevices << " MANDO" << (numDevices > 1 ? "S" : "") << " DETECTADO";
	else
		deviceInfo << "NO SE DETECTARON MANDOS";
	...
	// message
	mMsg1 = std::make_shared<TextComponent>(mWindow, "MANTEN PUSLSADO UN BOTON EN TU MANDO PARA CONFIGURARLO.", Font::get(FONT_SIZE_SMALL), 0x777777FF, ALIGN_CENTER);
	mGrid.setEntry(mMsg1, Vector2i(0, 2), false, true);

	const char* msg2str = firstRun ? "PULSA F4 PARA SALIR EN CUALQUIER MOMENTO." : "PULSA ESC PARA CANCELAR.";

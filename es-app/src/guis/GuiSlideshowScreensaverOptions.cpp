...
GuiSlideshowScreensaverOptions::GuiSlideshowScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel(row, "ESTIRAR IMAGENES", sss_stretch);
	...
	addWithLabel(row, "USAR IMAGENES PERSONALIZADAS", sss_custom_source);
	...
	addEditableTextComponent(row, "DIRECTORIO DE IMAGENES PERSONALIZADAS", sss_image_dir, Settings::getInstance()->getString("SlideshowScreenSaverImageDir"));
	...
	addWithLabel(row, "DIRECTORIO DE IMAGENES PERSONALIZADAS RECURSIVO", sss_recurse);
